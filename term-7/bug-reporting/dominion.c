...
void salvager_card(struct gameState* state , int currentPlayer, int choice, int handPos)
{
    ...
    if(choice)
    {
        ...
        discardCard(choice, currentPlayer, state, 0);
    }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  {
  case adventurer:
    adventurer_card(state, currentPlayer, drawntreasure, z, temphand);
    /*
    while (drawntreasure < 2)
    {
      if (state->deckCount[currentPlayer] < 1)
  ...
  case salvager:
    salvager_card(state, currentPlayer, choice1, handPos);
    /*
    //+1 buy
    state->numBuys++;
  ...
  case treasure_map:
    return treasure_map_card(state, currentPlayer, handPos);
