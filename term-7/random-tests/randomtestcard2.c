...
// put village in hand, copy game state, test results
void testVillage(int player, struct gameState *post, int *passedAllTests) {
    ...
    // Place village in random hand position.
    ...
    // Check number of actions before and after playing village
    if(NOISY_TEST){printf("Actions before playing village: %d, should gain: %d, number after: %d\n", pre.numActions, actionsGained, post->numActions);}
    myAssert(post->numActions == (pre.numActions + actionsGained), passedAllTests, "Wrong number of actions");

    // print state of hand, discard, deck, and number of cards that should be drawn
    if(NOISY_TEST){printf("Cards in hand before playing Village: %d\n", pre.handCount[player]);}
    ...
    // check number of cards in hand after playing village
    ...
    //  check that village is in played card pile
    if(NOISY_TEST){printf("Last card in played cards: %s, Expected: village\n", getCardName(post->playedCards[pre.playedCardCount]));}
    myAssert(post->playedCards[pre.playedCardCount] == village, passedAllTests, "village not in played card pile");
...
int main () {
  ...
  for (n = 0; n < NUM_TESTS; n++) {
    ...
    // Make sure there is room for village to be added to played Cards
    ...
    testVillage(player, &state, &passedAllTests);
  }
