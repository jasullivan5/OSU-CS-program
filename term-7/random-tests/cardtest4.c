...
int main(){
    ...
    for (i = 0; i < numPlayers; i++){
        ...
        myAssert(state.playedCardCount == numPlayedCards, &passedAllTests, "Wrong number of played cards");
        numPlayedCards++;
        ...
        myAssert(testState.playedCardCount == numPlayedCards, &passedAllTests, "Wrong number of played cards");
        ...
        myAssert(testState.deckCount[player] == (state.deckCount[player] - villageDraw), &passedAllTests, "Wrong deck count");
        ...
        cardsInHand = cardsInHand - 1 + villageDraw;
        ...
        //Top cards of player 1's deck
        printf("Top card of deck before playing test card:\n");
        for (j = topDeckIndex; j > topDeckIndex - villageDraw; j--){
        ...
        for (j = 0; j < testState.handCount[player]; j++){
            ...
            if (j + 1 < testState.handCount[player]){
                myAssert(testState.hand[player][j + 1] == state.deck[player][state.deckCount[player] - j - 1], &passedAllTests, "Wrong cards");
            }
            else{
                myAssert(testState.hand[player][0] == state.deck[player][state.deckCount[player] - j - 1], &passedAllTests, "Wrong cards");
            }
        }

        // Confirm number of actions has increased by 2
        printf("Actions before playing test card: %d\n", state.numActions);
        printf(" Actions after playing test card: %d\n", testState.numActions);
        myAssert(testState.numActions == state.numActions + villageActions, &passedAllTests, "Wrong action count");
