...
int main(){
    ...
    for (i = 0; i < numPlayers; i++){
        ...
        myAssert(testState.playedCards[numPlayedCards - 1] == TEST_CARD, &passedAllTests, "Played cards don't match");

        /****** Confirm that drawn cards come from player 1's deck and end up in player 1's hand ******/
        int greatHallDraw = 1;
        int greatHallActions = 1;
        ...
        printf("Player %d cards in deck after playing test card: %d\n", player + 1, testState.deckCount[player]);
        myAssert(testState.deckCount[player] == (state.deckCount[player] - greatHallDraw), &passedAllTests, "Wrong deck count");
        ...
        cardsInHand = cardsInHand - 1 + greatHallDraw;
        ...
        for (j = topDeckIndex; j > topDeckIndex - greatHallDraw; j--){
        ...
        myAssert(testState.numActions == state.numActions + greatHallActions, &passedAllTests, "Wrong number of actions");
