 ...
 * START DATE:	3/9/2018
 * DESCRIPTION: Sends a plaintext and key for one time pad encryption to a
 * 		daemon-like server program that performs the encryption. Then,
 * 		receives the cyphertext from the daemon and writes it to stdout.
 *****************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void getFileText(char* fileName, char* buffer) {
	...
	if (file == NULL) { fprintf(stderr, "otp_enc: ERROR, failed to open file\n"); exit(1); }
	...
	while ( (charsRead < (BUFFER_SIZE - 1)) && ((character = fgetc(file)) != EOF) ) {
		...
		else { fprintf(stderr, "otp_enc: ERROR, invalid characters detected in '%s'\n", fileName); exit(1); }
	}
	fclose(file);
}
...
int main(int argc, char *argv[]) {
	...
	if (strlen(key) < strlen(plaintext)) { fprintf(stderr, "otp_enc: ERROR, key must be at least as long as plaintext"); exit(1); }
	...
	if (serverHostInfo == NULL) { fprintf(stderr, "otp_enc: ERROR, no such host on port: %d\n", portNumber); exit(2); }
	memcpy((char*)&serverAddress.sin_addr.s_addr, (char*)serverHostInfo->h_addr, serverHostInfo->h_length); // Copy in the address

	// Set up the socket
	int socketFD = socket(AF_INET, SOCK_STREAM, 0); // Create the socket
	if (socketFD < 0) { fprintf(stderr, "otp_enc: ERROR opening socket on port: %d\n", portNumber); exit(2); }
	
	// Connect to server, by connecting socket to address
	if (connect(socketFD, (struct sockaddr*)&serverAddress, sizeof(serverAddress)) < 0) { fprintf(stderr, "otp_enc: ERROR connecting on port: %d\n", portNumber); exit(2); }



	// Send handshake protocol to confirm for server that connecting process is otp_enc
	...
	if ((stringLength > 0) || (charsWritten < 0)) { fprintf(stderr, "otp_enc: ERROR writing to socket on port: %d\n", portNumber); exit(2); }


	// Load plaintext, key, and end transmission character into buffer
	...
	// send plaintext, key, and end transmission character to otp_enc_d
	...
	if (checkSend < 0) { fprintf(stderr, "otp_enc: ioctl error on port %d\n", portNumber); exit(2); } // Make sure loop wasn't exited due to error



	// Get cyphertext back from otp_enc_d
	...
	if (charsRead < 0) { fprintf(stderr, "otp_enc: ERROR reading from socket on port: %d\n", portNumber); exit(2); }
	if (buffer[strlen(buffer) - 1] != END_TRANSMISSION) { fprintf(stderr, "otp_enc: ERROR, did not receive end transmission code on port: %d\n", portNumber); exit(2); }
	buffer[strlen(buffer) - 1] = '\n'; // Replace end transmission character with newline character in cyphertext
	printf("%s", buffer); // write cyphertext to stdout
